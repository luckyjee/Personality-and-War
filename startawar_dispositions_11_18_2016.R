...
##############
## 11-20-2016
## Tried with clustered SE for Extraversion, but the results are not as stark. 
...
start_exthi_mod$clse<-cluster.vcov(start_exthi_mod,start_stack[start_stack$extraversionHi==1,]$resp_id)
seh<-coeftest(start_exthi_mod, start_exthi_mod$clse)
seh

seh_vars=variable.names(start_exthi_mod)
seh_coefs = c(seh[,1])
...
seh_tmp2=seh_tmp
seh_tmp2$name = row.names(seh_tmp2)
seh_tmp2 = seh_tmp2[,c("name","seh_coefs","seh_err")]
names(seh_tmp2) = c("variable","coef","se")

start_exthi_tmp3<-seh_tmp2[-1,]
...
start_extlo_mod$clse<-cluster.vcov(start_extlo_mod,start_stack[start_stack$extraversionLo==1,]$resp_id)
sel<-coeftest(start_extlo_mod, start_extlo_mod$clse)
sel

sel_vars=variable.names(start_extlo_mod)
sel_coefs = c(sel[,1])
sel_err = c(sel[,2]*1.96)
sel_tvals = c(e[,4])
sel_tmp = data.frame(cbind(sel_coefs,sel_err))



## select particular coefficients
start_extlo_tmp2=sel_tmp
start_extlo_tmp2$name = row.names(start_extlo_tmp2)
start_extlo_tmp2 = start_extlo_tmp2[,c("name","sel_coefs","sel_err")]
